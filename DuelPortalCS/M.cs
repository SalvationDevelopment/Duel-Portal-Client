...
using System.Windows;
    ...
    public static class M{
       ...
       // public static DuelPortalCS.SQLReference.CardDetails[] OpponentHand = new CardDetails[21];
        ...
        public static HashSet<string> PoolOptions = new HashSet<string>() { "Default", "LCCG", "HCCG", "ACG", "YDF" };
        ...
        public static bool InBetween(this int i, int low, int high)
        {
            return i >= low && i <= high;
        }
        ...
        public static string toProperImageNameNoExtension(int fakeid)
        {
            if (isDefaultSet)
                return M.CardStats[fakeid].ID.ToString();
            else
                return M.mySet + "_" + M.CardStats[fakeid].Name;
        }
       ...
       // public static Dictionary<Image, Image> RealToDummyImage = new Dictionary<Image, Image>();
        ...
        public static void SendErrorReport(string errorClass, string errorMessage, object result)
        {
            ...
            client.WriteErrorReportAsync(errorClass.Replace("DuelPortalCS.SQLReference.", ""), errorMessage, result, M.username);
        }
        ...
        public static void ParseSETCards(string data)
        {
            ...
            do
            {
                ...
                for (int n = 1; n <= 11; n++)
                {
                    if (string.IsNullOrEmpty(lines[linenum - 1]))
                    { return;}

                    rbar = lines[linenum - 1].IndexOf("|", lbar + 1, StringComparison.InvariantCultureIgnoreCase);
                    if (rbar <= 1)
                    {
                        if (string.IsNullOrEmpty(statBuild.Name)) //A VERY Dumb bug that I have no explanation for!!!
                            rbar = lines[linenum - 1].IndexOf("|", 1);
                        else
                            break;
                    }
                ...
                //if (string.IsNullOrEmpty(statBuild.Name))
                //    continue;
            ...
            for (int n = 0; n < lines.Length; n++)
            {
                switch (lines[n].Trim())
                {
                    ...
                    case "~Semi-Limited~":
                        ...
                        continue;
                    default:
                        int id = M.findID(lines[n].Trim());
                        if (id > 0)
                           M.CardStats[id].Limit = currentLimit; 
                        break;
                }
            }
        }
        ...
        public static bool isOwner(int id)
        {
            ...
            string cardCreatorString = M.CardStats[id].Creator.Replace("_", "");
            ...
            if (string.Compare(ownerString, M.username, StringComparison.OrdinalIgnoreCase) == 0)
            {
                return true;
            }
            else
        ...
        {
            return formatYCM(M.CardStats[id], picture);
        }
        public static string formatYCM(DuelPortalCS.SQLReference.CardDetails stats, string picture)
        {
            ...
            if (theCard.Type.Contains("Tuner") && !theCard.Type.Contains("Effect"))
                newUrl.Append("type=" + M.getFirstType(theCard.Type) + " / Tuner&");
            else
                newUrl.Append("type=" + M.getFirstType(theCard.Type) + "&");
